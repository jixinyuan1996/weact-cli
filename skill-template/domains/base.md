> **命名约定：** Shortcut 命令组和原生 API / schema 都使用 `weact-cli base ...`。
> **分流规则：** 如果用户要把本地文件导入成 Base / 多维表格 / bitable，第一步不是 `base`，而是 `weact-cli drive +import --type bitable`。

## 核心规则

1. **原生 API 命令路径必须完整** — 资源名是多级点分路径，不能省略前缀
   - `weact-cli base table.records create` ✅
   - `weact-cli base records create` ❌
...
8. **不要走旧 bitable 路径** — Base 场景不要调用 `weact-cli api GET /open-apis/bitable/v1/...`；即使 wiki 解析结果是 `obj_type=bitable`，后续也应继续使用 `weact-cli base ...`
9. **不要把本地文件导入误判成 Base 表内操作** — 如果目标是“把 Excel / CSV / `.base` 快照导入成 Base / 多维表格”，必须先走 `weact-cli drive +import --type bitable`；只有导入完成后，才回到 `weact-cli base ...`
...
| 把本地 Excel / CSV / `.base` 导入为 Base / 多维表格 | `weact-cli drive +import --type bitable` | 导入阶段属于 `drive`，不是 `base` |
...
- **附件字段**：上传本地文件时只能走 `weact-cli base +record-upload-attachment`
...
1. **使用 `wiki.spaces.get_node` 查询节点信息**
   ```bash
   weact-cli wiki spaces.get_node --params '{"token":"&lt;wiki_token&gt;"}'
   ```
...
3. **根据 `obj_type` 选择后续命令**
   ...
   | `sheet` | 电子表格 | `sheets.*` |
   | `bitable` | 多维表格 | `weact-cli base +...`（优先）；如果 shortcut 不覆盖，再用 `weact-cli base <resource> <method>`；不要改走 `weact-cli api /open-apis/bitable/v1/...` |
   | `slides` | 幻灯片 | `drive.*` |
   | `file` | 文件 | `drive.*` |
   | `mindnote` | 思维导图 | `drive.*` |
...
5. **如果已经报了 token 错，再回退检查 wiki**
   ...
   - 确认 `obj_type=bitable` 后，用 `node.obj_token` 重试 `weact-cli base ...`

### 查询示例

```bash
# 查询 wiki 节点
weact-cli wiki spaces.get_node --params '{"token":"Pgrrwvr***********UnRb"}'
```

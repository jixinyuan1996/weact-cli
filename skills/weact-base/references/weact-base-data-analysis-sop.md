...
- `+data-query`: entry guide [weact-base-data-query-guide.md](weact-base-data-query-guide.md), full DSL SSOT [weact-base-data-query.md](weact-base-data-query.md)
- 视图筛选: [weact-base-view-set-filter.md](weact-base-view-set-filter.md)
...
`filter-json` 与视图筛选结构一致。下面只列常用 fewshot；字段类型、operator、value 形状拿不准，或需要人员、群组、关联、空值、地理位置、formula / lookup 等完整筛选时，先读 [weact-base-view-set-filter.md](weact-base-view-set-filter.md)，再把同样的 filter JSON 传给 `--filter-json`。
...
- 常用聚合 fewshot 先读 [weact-base-data-query-guide.md](weact-base-data-query-guide.md)；字段类型、日期 value、DSL shape 以 [weact-base-data-query.md](weact-base-data-query.md) 为准。

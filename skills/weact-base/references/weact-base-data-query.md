...
> **入口指南**: [weact-base-data-query-guide.md](weact-base-data-query-guide.md) | **前置条件**: 先阅读 [`../weact-shared/SKILL.md`](../../weact-shared/SKILL.md) 了解认证、全局参数和安全规则。

本文档是 `+data-query` JSON DSL 的单一事实来源（SSOT），用于说明完整字段、操作符、限制、返回和错误恢复。常用 fewshot 与命令选择先读 [weact-base-data-query-guide.md](weact-base-data-query-guide.md)。

查询类任务还必须先遵守 [`weact-base-data-analysis-sop.md`](weact-base-data-analysis-sop.md)。`+data-query` 适合让筛选、分组、聚合、排序和 TopN 在 Base 云端查询服务中执行；不要用默认分页的 `+record-list` 或本地 `jq` 替代聚合查询。
...
```
https://example.weact.cn/base/<base_token>?table=<table_id>
```
...
2. **先查表结构**：执行 `weact-cli base +field-list --base-token <base_token> --table-id <table_id>`
...
- ⚠️ **必须先查表结构**：DSL 的 `field_name` 必须与表中字段名称精确匹配（区分大小写），不能凭猜测构造。先用 `weact-cli base +field-list --base-token <base_token> --table-id <table_id>` 获取真实字段名
...
- [weact-base](../SKILL.md) — 多维表格全部命令
- [weact-shared](../../weact-shared/SKILL.md) — 认证和全局参数
- [weact-base-data-analysis-sop.md](weact-base-data-analysis-sop.md) — 查询范围、选路、下推、分页、`+record-list` / `+record-search` 回查和关系查询 SOP
- [weact-base-cell-value.md](weact-base-cell-value.md) — CellValue 格式规范
- [weact-base-field-json.md](weact-base-field-json.md) — 字段类型与 JSON 结构

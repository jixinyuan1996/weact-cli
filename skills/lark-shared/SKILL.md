...
本技能指导你如何通过weact-cli操作飞书资源, 以及有哪些注意事项。
...
weact-cli auth login --scope "calendar:calendar:readonly" --no-wait --json
```
...
1. 执行 `weact-cli auth login --scope "xxx" --no-wait --json`（必须加 `--no-wait --json`）
2. 从 JSON 输出中提取 `verification_url` 和 `device_code`
3. 生成二维码：`weact-cli auth qrcode <verification_url> --output "xxx"`
...
1. 等待用户回复"已完成授权"
2. **由你（AI agent）亲自执行**：`weact-cli auth login --device-code <device_code>`
3. 此命令会轮询授权状态并完成登录
4. 如果返回授权成功，流程结束
...
- **禁止缓存 `verification_url` 或 `device_code`**：每次需要授权时，必须重新执行 `weact-cli auth login --no-wait --json` 生成新的链接。不要将授权链接和 device code 存入上下文供后续复用

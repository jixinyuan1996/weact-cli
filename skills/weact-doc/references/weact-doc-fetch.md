...
`content` 的格式由 `--doc-format` 决定；`im-markdown` 仅用于获取内容后在 `weact-im` 场景下使用。设置 `--scope` 时会被 `<fragment>` 包裹，详见上文"局部读取的输出结构"。
...
| `--doc-format` | 否 | `xml`（默认）\| `markdown` \| `im-markdown`（仅用于获取内容后在 `weact-im` 场景下使用） |
...
返回中可能含 `<sheet>`、`<bitable>`、`<cite file-type="sheets|bitable">`。内部数据无法通过 `docs +fetch` 获取，提取 `token` 等属性后切到 [`weact-sheets`](../../weact-sheets/SKILL.md) / [`weact-base`](../../weact-base/SKILL.md) 下钻，详见 [SKILL.md 快速决策](../SKILL.md) 路由表。

## 参考

- [weact-doc-create](weact-doc-create.md) — 创建文档
- [weact-doc-update](weact-doc-update.md) — 更新文档
- [weact-doc-media-preview](weact-doc-media-preview.md) — 预览素材
- [weact-doc-media-download](weact-doc-media-download.md) — 下载素材/画板缩略图
- [weact-doc-resource-cover](weact-doc-resource-cover.md) — 读取、更新、删除文档封面图

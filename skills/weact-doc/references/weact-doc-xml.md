...
- `<whiteboard>` — 简单图由 SubAgent 直接插入 `<whiteboard type="svg">完整自包含 SVG</whiteboard>`；复杂图使用 `<whiteboard type="blank"></whiteboard>` 先创建空白画板，再按 [`weact-doc-whiteboard.md`](weact-doc-whiteboard.md) 启动 SubAgent 调用 `weact-whiteboard` 写入；
...
> 详见 [weact-doc-update.md](weact-doc-update.md)。
...
- 当只有纯文本名字而没有 `open_id` 时（如系统消息、合并转发内容），先通过 `weact-cli contact +search-user --query "名字" --as user` 反查 `open_id`，再写入 cite 标签。

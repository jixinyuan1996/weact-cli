...
4. Spawn Agent 并行撰写各章节。每个 Agent 需收到：
   ...
   - `weact-doc-xml.md` 和 `weact-doc-style.md` 的完整路径（Agent 须先读取）
...
7. **画板意图识别**：逐章节扫描，按 `weact-doc-style.md`「画板意图识别」表判断是否有段落适合用图表达。重要信息优先画板化，记录需要插图的章节、推荐画板类型、mermaid/SVG 路径和用于画图的源内容

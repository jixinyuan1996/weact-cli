...
| Skill             | 核心职责                                                      | 约束                              |
|-------------------|-----------------------------------------------------------|---------------------------------|
| `weact-doc`        | 识别画板机会、使用 Mermaid/SVG 创建图表、调度 SubAgent、插入简单 SVG 画板或复杂空白画板 | 主 Agent 不直接创作画板内容；              |
| `weact-whiteboard` | 查询/导出已有画板；复杂图表生成（Mermaid/DSL/SVG 路由、场景选型、渲染验证）；写入已有/空白画板  | 仅特别复杂的图表或已有画板更新时由独立 SubAgent 读取 |
...
| 只查看 / 下载已有画板            | 切换至 `weact-whiteboard`，不走本流程                               |
...
- 图表目标、受众、源段落或数据
- 要求读取 `weact-doc-xml.md`；不需要读取 `weact-whiteboard`
...
插入画板后，可以从返回值使用 weact-cli 指令，将画板内容导出为 png
图片。若是对设计不满意，可以修改后，删除原来的画板再重新插入，或是调用 [
`../../weact-whiteboard/SKILL.md`](../../weact-whiteboard/SKILL.md) 编辑。
...
### 步骤 3B：编辑已有画板 — 启动 weact-whiteboard SubAgent

复杂图和已有画板更新必须启动 SubAgent。主 Agent 只传最小上下文，不直接执行 `weact-whiteboard` 的渲染和写入流程。
...
- 图表目标、推荐画板类型、受众
- 与图表直接相关的源段落或数据
- 要求读取 [`../../weact-whiteboard/SKILL.md`](../../weact-whiteboard/SKILL.md)，按其完整流程写入该 board_token
...
- 画板查询/创作/修改/渲染写入：[`../../weact-whiteboard/SKILL.md`](../../weact-whiteboard/SKILL.md)

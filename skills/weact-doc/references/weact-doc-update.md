...
> 1. [`weact-doc-xml.md`](weact-doc-xml.md) — XML 语法规则（使用 Markdown 格式时改读 [`weact-doc-md.md`](weact-doc-md.md)）
> 2. [`weact-doc-style.md`](style/weact-doc-style.md) — 排版指南（元素选择、丰富度规则、颜色语义）
> 3. [`weact-doc-update-workflow.md`](style/weact-doc-update-workflow.md) — 改写增强工作流（Code-Act Loop、并行执行策略）
>
> **未读完以上文件就生成内容会导致格式错误。**
...
| `warnings` | 警告信息列表 |
| `document.new_blocks` | 本次操作新增的 block 列表（如画板）。`block_id` 可用于后续精确编辑；`block_token` 是资源块 token（如画板）可交给 `weact-whiteboard` 等 skill 继续操作 |
...
> **`docs +update` 不能直接编辑已有画板的内容。** 本命令只能**新增**画板块；要修改已有画板，先用 `docs +fetch` 取到 `<whiteboard token="...">`，再按 [`weact-doc-whiteboard.md`](weact-doc-whiteboard.md) 启动 SubAgent 读取 [`weact-whiteboard`](../../weact-whiteboard/SKILL.md) 并写入。

画板的语法选型与插入示例见 [`weact-doc-style.md`](style/weact-doc-style.md) 的「画板语法与插入」章节。
...
- **表达形式**：插入或替换内容时，优先沿用用户要求和已有文档风格；需要结构化表达时可参考 [`weact-doc-style.md`](style/weact-doc-style.md)，但不要为了固定丰富度主动添加组件

## 参考

- [`weact-doc-update-workflow.md`](style/weact-doc-update-workflow.md) — 改写增强工作流（Code-Act Loop、并行执行策略）
- [`weact-doc-style.md`](style/weact-doc-style.md) — 文档样式指南（元素选择 + 丰富度规则 + 颜色语义）
- [`weact-doc-xml.md`](weact-doc-xml.md) — XML 语法规范
- [`weact-doc-fetch.md`](weact-doc-fetch.md) — 获取文档
- [`weact-doc-create.md`](weact-doc-create.md) — 创建文档
- [`weact-doc-media-insert.md`](weact-doc-media-insert.md) — 插入图片/文件到文档

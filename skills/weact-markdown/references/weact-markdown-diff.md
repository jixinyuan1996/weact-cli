...
比较 Drive 中原生 Markdown 的两个历史版本，或比较远端 Markdown 与本地 `.md` 草稿。需要历史版本号时，先用 [`drive +version-history`](../../weact-drive/references/weact-drive-version-history.md) 获取 `version`，不要使用 `tag`。

## 命令

```bash
# 比较两个远端版本
weact-cli markdown +diff \
...
- [weact-markdown](../SKILL.md) — Markdown 域总览
- [weact-drive-version-history](../../weact-drive/references/weact-drive-version-history.md) — 获取可用于 diff 的历史版本号

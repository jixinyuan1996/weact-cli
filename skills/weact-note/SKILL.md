...
name: weact-note
version: 1.0.0
description: "WeAct会议纪要（Note）直查：已知 note_id 时查询纪要详情、展示类型、关联文档 token，并读取 unified 原始逐字记录。当用户已持有 note_id，或从文档显式 vc-node-id 获得 note_id 时使用。不负责会议/日程/妙记定位、文档标题搜索或 Docx 正文读取。"
metadata:
  requires:
    bins: ["weact-cli"]
  cliHelp: "weact-cli note --help"
---

# note (v1)

身份：仅使用 `--as user`。使用前阅读 [`../weact-shared/SKILL.md`](../weact-shared/SKILL.md)。
...
- `note_doc_token`：纪要正文文档，正文读取交给 [weact-doc](../weact-doc/SKILL.md)。
- `verbatim_doc_token`：普通纪要逐字稿文档；unified 逐字稿不按这个 token 路由。

## 不在本 Skill 范围

- 通过 `meeting_id` 定位纪要（`note_id`）→ [weact-vc](../weact-vc/SKILL.md)（`vc +detail`）。
- 通过 `minute_token` 定位纪要（`note_id`）→ [weact-minutes](../weact-minutes/SKILL.md)（`minutes +detail` 顶层返回 `note_id`）。
- 通过日程 `event_id` 定位会议(`meeting_id`) / 用户绑定纪要(`meeting_note`) → [weact-calendar](../weact-calendar/SKILL.md)（`calendar +meeting`）。
- 自然语言纪要标题搜索 → [weact-drive](../weact-drive/SKILL.md) / [weact-doc](../weact-doc/SKILL.md)。
- Docx 正文读取 → [weact-doc](../weact-doc/SKILL.md)。
- 妙记基础信息与媒体文件 → [weact-minutes](../weact-minutes/SKILL.md)。

## Shortcuts

| Shortcut | 何时读 reference |
|----------|------|
| [`+detail`](referenc../weact-note-detail.md) | 需要解释输出字段或根据展示类型继续路由 |
| [`+transcript`](referenc../weact-note-transcript.md) | 需要拉取 unified 原始记录或处理本地输出文件 |

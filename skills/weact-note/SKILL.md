...
- `note_doc_token`：纪要正文文档，正文读取交给 [lark-doc](../weact-doc/SKILL.md)。
- `verbatim_doc_token`：普通纪要逐字稿文档；unified 逐字稿不按这个 token 路由。

## 不在本 Skill 范围

- 通过 `meeting_id` 定位纪要（`note_id`）→ [lark-vc](../weact-vc/SKILL.md)（`vc +detail`）。
- 通过 `minute_token` 定位纪要（`note_id`）→ [lark-minutes](../weact-minutes/SKILL.md)（`minutes +detail` 顶层返回 `note_id`）。
- 通过日程 `event_id` 定位会议(`meeting_id`) / 用户绑定纪要(`meeting_note`) → [lark-calendar](../weact-calendar/SKILL.md)（`calendar +meeting`）。
- 自然语言纪要标题搜索 → [lark-drive](../weact-drive/SKILL.md) / [lark-doc](../weact-doc/SKILL.md)。
- Docx 正文读取 → [lark-doc](../weact-doc/SKILL.md)。
- 妙记基础信息与媒体文件 → [lark-minutes](../weact-minutes/SKILL.md)。

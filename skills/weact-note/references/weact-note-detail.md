...
- 可以来自用户直接给出的 `note_id`。
- 如果入口是文档，先由 [weact-doc](../../weact-doc/SKILL.md) 读取 Docx；只有 `<vc-transcribe-tab vc-node-id="...">` 的 `vc-node-id` 可以作为 `note_id`。
...
| `note_display_type=unified` | 读逐字稿 / 原始记录：转 [`note +transcript`](weact-note-transcript.md) |

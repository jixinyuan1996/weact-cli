...
按 release ID 查询单次发布详情。运行时命令事实以 `weact-cli apps +release-get --help` 为准。

## 何时用

用于跟进已知 `release_id` 的发布状态。没有 `release_id` 时先读 [`weact-apps-release-list.md`](weact-apps-release-list.md)，不要让用户手填。

`release_id` 是妙搭发布 ID（`+release-create` 返回），不是飞书审批实例号；查发布进度/失败都在 `apps +release-*` 命令族内完成，不要路由到 weact-approval。
...
```bash
weact-cli apps +release-get --app-id app_xxx --release-id release_yyy
```

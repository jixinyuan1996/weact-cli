...
> **前置条件：** 先阅读 [`../weact-shared/SKILL.md`](../../weact-shared/SKILL.md)（认证 / 全局参数 / 安全）。
...
```bash
weact-cli apps +env-pull --app-id app_xxx
```

## 失败处理

`missing_scope`（没拿到 `spark:app:read`）时，按 weact-shared 引导 `weact-cli auth login --domain apps`。其余失败优先转述 `error.hint` / `error.message`。

## 参考

- [weact-apps](../SKILL.md) — 妙搭应用全部命令 + 心智模型
- [weact-apps-local-dev](weact-apps-local-dev.md) — 本地全栈开发端到端流程

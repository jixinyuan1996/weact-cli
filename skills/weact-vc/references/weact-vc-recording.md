...
本 skill 对应 shortcut：`weact-cli vc +recording`。

## 命令

```bash
# 通过会议 ID 查询（逗号分隔支持批量，最多 50 个）
weact-cli vc +recording --meeting-ids 69xxxxxxxxxxxxx28
weact-cli vc +recording --meeting-ids 69xxxxxxxxxxxxx28,69xxxxxxxxxxxxx29

# 通过日程事件 ID 查询
weact-cli vc +recording --calendar-event-ids xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx_0

# 输出格式
weact-cli vc +recording --meeting-ids 69xxxxxxxxxxxxx28 --format json

# 预览 API 调用
weact-cli vc +recording --meeting-ids 69xxxxxxxxxxxxx28 --dry-run
```
...
该命令仅支持 `user` 身份，使用前需完成 `weact-cli auth login`。user token 只能查自己有权限的录制。
...
| 输入参数 | 获取方式 |
|---------|---------|
| `meeting_id` | 使用 `weact-cli vc +search` 搜索历史会议，取结果中的 `id` 字段 |
| `calendar_event_id` | 使用 `weact-cli calendar +agenda` 查看日程，取结果中的 `event_id` 字段 |
...
weact-cli vc +recording --meeting-ids xxx

# 第 2 步：使用上一步返回的 minute_token 下载妙记文件
weact-cli minutes +download --minute-tokens <minute_token>
```
...
weact-cli minutes +detail --minute-tokens <minute_token> --summary --todo --chapter --transcript
```
...
weact-cli vc +search --query "周会" --start 2026-03-10

# 第 2 步：使用上一步返回的 meeting_ids 查询录制，拿到 minute_tokens
weact-cli vc +recording --meeting-ids <ids>

# 第 3 步：使用其中一个 minute_token 下载妙记文件
weact-cli minutes +download --minute-tokens <token>
```
...
weact-cli vc +recording --calendar-event-ids <event_id>
...
- `minute_token` 从录制 URL 尾段解析（`https://meetings.weact.cn/minutes/{minute_token}`）。
...
- [weact-vc](../SKILL.md) — 视频会议全部命令
- [weact-vc-search](weact-vc-search.md) — 搜索历史会议（获取 meeting_id）
- [weact-minutes-detail](../../weact-minutes/references/weact-minutes-detail.md) — 获取会议纪要

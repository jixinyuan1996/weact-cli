...
`vc +search` 只能搜索已结束的历史会议记录，不用于查询未来日程。查询未来会议安排请使用 [weact-calendar](../../weact-calendar/SKILL.md)。
...
| 搜不到未来会议 | `vc +search` 只查历史会议 | 改用 [weact-calendar](../../weact-calendar/SKILL.md) 查询未来日程 |

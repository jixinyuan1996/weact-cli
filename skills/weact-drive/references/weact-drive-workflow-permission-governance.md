# weact-drive 权限治理 Workflow

Workflow id: `permission_governance`

Risk / Structure: `R2` / `S2`

本文实现已注册的权限治理 workflow。执行前必须先读取 [`weact-drive-workflow.md`](weact-drive-workflow.md) 和 [`../../weact-shared/SKILL.md`](../../weact-shared/SKILL.md)，并遵循共享执行协议、Artifact Contract、Workflow Loading、认证和写入确认规则。
...
本表只规定每个 state 需要加载的额外上下文；命令可用范围以 `Command Map` 为准。需要拼装具体 `weact-cli` 命令时，再按需读取 [`weact-drive-workflow-permission-governance-commands.md`](weact-drive-workflow-permission-governance-commands.md)。

| State | Required Reference |
|-------|--------------------|
| `PARSE_INTENT` | 本文件、[`weact-drive-workflow.md`](weact-drive-workflow.md)、[`../../weact-shared/SKILL.md`](../../weact-shared/SKILL.md) |
| `TARGET_INSPECT` | [`weact-drive-inspect.md`](weact-drive-inspect.md) |
| `DISCOVER_TARGETS` | 容器范围时读取 [`../../weact-wiki/references/weact-wiki-node-list.md`](../../weact-wiki/references/weact-wiki-node-list.md) 或 [`weact-drive-files-list.md`](weact-drive-files-list.md) |
...
| `EXEC_CONFIRM` | 只为用户选择的动作读取 [`weact-drive-apply-permission.md`](weact-drive-apply-permission.md)、[`weact-drive-secure-label.md`](weact-drive-secure-label.md)，或 `weact-cli schema drive.permission.public.patch` / `weact-cli schema drive.permission.members.transfer_owner`；需要确认模板时读取 [`weact-drive-workflow-permission-governance-outputs.md`](weact-drive-workflow-permission-governance-outputs.md) |
...
本入口不内联命令样例。需要拼装具体 `weact-cli` 命令时，按当前 state 读取 [`weact-drive-workflow-permission-governance-commands.md`](weact-drive-workflow-permission-governance-commands.md)。命令是否允许执行仍以 `Command Map` 和写入规则为准。
...
2. 按 [`weact-drive-files-list.md`](weact-drive-files-list.md) 递归处理 `data.files`、`has_more` 和 `next_page_token`。不要把第一页数量当作完整范围。
...
每个可审计目标都必须先归一化为 `per_target_permission_assessment`，再按 [`weact-drive-workflow-permission-governance-outputs.md`](weact-drive-workflow-permission-governance-outputs.md) 的 `Semantic Rendering` 渲染。`public_exposure_check` 只是 `target_count=1` 的轻量渲染模式；它和多目标、容器诊断复用同一套语义字段与风险分类。该判断只覆盖当前文档公共访问和协作权限设置，不审计协作者名单、历史权限变更、完整继承链或审计日志。
...
- 面向用户优先使用业务语言和“候选风险 / 待复核 / 待策略确认”；底层字段只作为证据。完整模板按需读取 [`weact-drive-workflow-permission-governance-outputs.md`](weact-drive-workflow-permission-governance-outputs.md)。

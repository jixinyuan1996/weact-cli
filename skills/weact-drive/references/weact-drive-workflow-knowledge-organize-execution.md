...
1. `active_plan_items` and `execution_scope` MUST already exist from [`weact-drive-workflow-knowledge-organize-planning.md`](weact-drive-workflow-knowledge-organize-planning.md).
...
6. Before writing to `my_library` root, resolve the real `space_id` according to `weact-wiki`.

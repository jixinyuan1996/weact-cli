...
> 前置条件：先阅读 [`../SKILL.md`](../SKILL.md) 的“评论能力入口”，添加评论参数细节见 [`weact-drive-add-comment.md`](weact-drive-add-comment.md)，reaction 见 [`weact-drive-reactions.md`](weact-drive-reactions.md)。
...
- 需要根据评论定位到文档正文位置时（例如根据评论 review 文档、区分多处相同引用文本、把评论落点映射到 `docs +fetch` 的 block），先确认目标是 `file_type=docx`，再阅读 [`weact-drive-comment-location.md`](weact-drive-comment-location.md)。
- 其他文档类型暂不支持返回定位字段。

...
- 如果 `quote` 是 `C3`、`A1` 这类单元格坐标，可拆出 `spreadsheet_token` / `sheet_id` 后用 `weact-sheets` 读取该单元格确认：
...
- 下钻读取时切到 `weact-base`，最少确认表、字段、记录：
...
- 若要定位画板内部节点，切到 `weact-whiteboard` 读取 raw 节点结构：

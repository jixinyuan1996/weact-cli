...
本文只提供 `permission_governance` workflow 的具体 `weact-cli` 命令样例。只有进入对应 state 且需要拼装命令时才读取本文；命令可用范围仍以 [`weact-drive-workflow-permission-governance.md`](weact-drive-workflow-permission-governance.md) 的 `Command Map` 为准。
...
```bash
weact-cli drive +inspect --url '<url>' --as user --format json
```
...
```bash
weact-cli drive metas batch_query \
...
```bash
weact-cli drive permission.public get \
...
```bash
weact-cli drive file.statistics get \
...
```bash
weact-cli drive file.view_records list \
...
```bash
weact-cli drive permission.members auth \
...
```bash
weact-cli schema drive.permission.public.patch --format json
```
...
```bash
weact-cli drive permission.public patch \
  --params '{"token":"<token>","type":"<type>"}' \
...
```bash
weact-cli drive +apply-permission \
  --token '<url>' \
...
```bash
weact-cli schema drive.permission.members.transfer_owner --format json
```

显式确认后转移 owner：

```bash
weact-cli drive permission.members transfer_owner \
...
```bash
weact-cli drive +secure-label-list \

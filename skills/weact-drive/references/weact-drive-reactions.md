...
> **前置条件：** 先阅读 [`../SKILL.md`](../SKILL.md) 了解 Drive 评论入口，再阅读 [`weact-drive-comments-guide.md`](weact-drive-comments-guide.md) 了解评论卡片模型、评论数/回复数统计口径、`file_token` / `file_type` 规则；同时阅读 [`../../weact-shared/SKILL.md`](../../weact-shared/SKILL.md) 了解认证、全局参数和安全规则。

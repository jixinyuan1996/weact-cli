...
> **人员筛选：不要猜 ID。** 不知道 `open_id` 时，先用 `lark-contact` 查 id：`weact-cli contact +search-user --query "<姓名/邮箱/手机号>" --as user`。
...
> **群组筛选：不要猜 ID。** 不知道 `chat_id` 时，先用 `lark-im` 搜群：`weact-cli im +chat-search --query "<群名关键词>" --as user`；取结果里的 `oc_xxx`。

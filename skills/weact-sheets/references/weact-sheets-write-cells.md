...
> **单元格图片 vs 浮动图片（最易选错）**：图若**属于某条记录、要随那行排序 / 筛选 / 增删**（凭证 / 证件照 / 每行配图，话里带「对应 / 每行 / 这列」等绑定词）→ **单元格图片**（本工具）：用 `+cells-set-image`（最短）或 `+cells-set` 的 `rich_text` + `type: "embed-image"`。只是自由摆放的装饰（logo / 水印 / 封面）→ 浮动图片，见 weact-sheets-float-image。别因「浮动图更好控制 / 更熟」默认选浮动图——它承载"对应某记录"的图会随增删行 / 排序错位。
...
💡 **写入公式前先按迁移规则改写**：如果公式来自 Excel 或包含数组场景，先读取并遵循 `weact-sheets-formula-translation` 的规则完成改写，再把最终公式写入 `formula` 字段。
...
4. **去重 / 筛选函数**：飞书**支持** `UNIQUE` / `FILTER` / `SPLIT`（原生数组函数，详见 `weact-sheets-formula-translation`），可直接用；`DISTINCT` 不是飞书函数，去重用 `UNIQUE`。大数据量去重 / 分组也可用透视表（`+pivot-{create|update|delete}`，值字段聚合方式选 count）
...
⚠️ **"汇总行"识别 → 读 `weact-sheets-visual-standards` 拿完整样式规范**：下述双重条件**同时满足**才是汇总行，禁止仅凭"有 AVERAGE"就判定：
...
满足上述时，**不要在本文里猜样式**，直接去读 `weact-sheets-visual-standards` 的「场景一 → 1A. 添加汇总行 / 表头行」章节，按那里的样式要点配齐 `font.bold / horizontal_alignment / background_color / border_styles`。
...
- **做法 A（推荐）**：按上方「内容与样式分离写入」两步法——先用模板单元格 + `--copy-to-range` 铺**完整样式**（`cell_styles` + `border_styles` 都要，不能只铺 border，否则新行字体 / 对齐 / 背景色全裸奔），再单独 `+cells-set` 写 value / formula。汇总行的 `cell_styles` 要点（bold / 背景色 / 上边框）见 `weact-sheets-visual-standards` 的「场景一 → 1A. 添加汇总行 / 表头行」。
...
带显示文本的超链接、@人、@文档这类富内容**必须**走 `+cells-set` 的 `rich_text` 字段（`cells[].rich_text` 数组，每段一个对象、带 `type`），**不能**直接传普通字符串——纯字符串只会被当作纯文本存进单元格。完整字段跑 `weact-cli sheets +cells-set --print-schema --flag-name cells`，常用段类型：

- **超链接（带显示文本）**：`{"type":"link","text":"飞书","link":"https://www.weact.cn"}`。纯 URL 不需要 `rich_text`，直接写普通字符串即可。
...
| **插行/列 + 写入** 这种多步组合，且要原子 | `+batch-update`（见 weact-sheets-batch-update） | 多次独立 `+cells-set`（非原子；插入会扰动后续 range） |
| 在**多个不连续 range** 上应用同一组样式 | `+cells-batch-set-style`（见 weact-sheets-batch-update） | 多次 `+cells-set-style`（非原子） |
...
weact-cli sheets +cells-set --url "https://example.weact.cn/sheets/shtXXX" \
  --sheet-name "Sheet1" --range "A1:B2" \
  --cells '[[{"value":"name"},{"value":"score"}],[{"value":"alice"},{"value":95}]]'

# 富 cell（公式 + 样式，cells 是二维矩阵每元素一个 cell schema）
weact-cli sheets +cells-set --spreadsheet-token shtXXX --sheet-id "$SID" \
...
```bash
# 加粗 + 黄底
weact-cli sheets +cells-set-style --url "..." --sheet-name "Sheet1" \
  --range "A1:B2" --font-weight bold --background-color "#FFFF00"

# 配套边框
weact-cli sheets +cells-set-style --url "..." --sheet-id "$SID" \
...
```bash
weact-cli sheets +cells-set-image --url "..." --sheet-name "Sheet1" \
...
```bash
# 内联 CSV
weact-cli sheets +csv-put --url "https://example.weact.cn/sheets/shtXXX" \
  --sheet-name "Sheet1" --start-cell "A1" \
  --csv $'name,score\nalice,95\nbob,87'

# 从文件
weact-cli sheets +csv-put --spreadsheet-token shtXXX --sheet-id "$SID" \
...
>
> ```bash
> weact-cli sheets +csv-put --url "..." --sheet-name "Sheet1" \
>   --start-cell "A1" \
>   --csv $'name,score\nalice,=SUM(B2:B10)'
...
> ```bash
> # 同样的统计块，结构化写入无需任何转义
> weact-cli sheets +cells-set --url "..." --sheet-name "Sheet1" --range "G4:H6" \
...
python export.py | weact-cli sheets +table-put --url "<表URL>" --sheets -
# 某 sheet 带 "mode":"append" 追加到已有数据末尾、默认不重复表头
weact-cli sheets +table-put --spreadsheet-token "<token>" --sheets @payload.json
```
...
```bash
weact-cli sheets +table-put --url "<表URL>" \

...
> **边界**：本文只讲"公式怎么写对"；公式**怎么写入表格**（`+cells-set` / 模板单元格 + `--copy-to-range` / 容错回读）见 `weact-sheets-write-cells` 与 `weact-sheets-core-operations`。本文不含 shortcut，铁律见 `weact-sheets-core-operations`。
...
> 本段是"飞书不支持函数"的**唯一权威清单**（`weact-sheets-core-operations` 不再单列，统一指向这里）。以下函数在飞书里不存在或被禁用，禁止主动使用；用户明确要求时应拒绝并提供替代方案：

...
> **前置条件：** 先阅读 [`../weact-shared/SKILL.md`](../weact-shared/SKILL.md) 了解认证、身份切换和安全规则。

当用户的需求**无法被现有 skill 或 CLI 已注册 API 覆盖**时，使用本技能从WeAct官方 markdown 文档库中逐层挖掘原生 OpenAPI 接口，然后通过 `weact-cli api` 裸调完成任务。

## 文档库结构

WeAct OpenAPI 文档以 markdown 层级组织：
...
| 品牌 | 入口 URL |
|------|----------|
| WeAct (Feishu) | `https://open.weact.cn/llms.txt` |
| WeAct | `https://open.weact.cn/llms.txt` |
...
- WeAct品牌使用 `open.weact.cn`
- WeAct 品牌使用 `open.weact.cn`
- 如不确定用户品牌，默认使用WeAct

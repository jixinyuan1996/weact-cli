...
# 创建 PPT + 添加 slide 页面
weact-cli slides +create --title "项目汇报" --slides '[
...
# 以应用身份创建（自动授权当前用户）
weact-cli slides +create --title "项目汇报" --as bot

# 预览（不执行）
weact-cli slides +create --title "项目汇报" --slides '[...]' --dry-run
```
...
>
> 如果演示文稿是**以应用身份（bot）创建**的，如 `weact-cli slides +create --as bot`，CLI 会**尝试为当前 CLI 用户自动授予该演示文稿的 `full_access`（可管理权限）**。
>
...
```bash
# 第 1 步：创建空白 PPT
PRES_ID=$(weact-cli slides +create --title "项目汇报" | jq -r '.data.xml_presentation_id')
...
- [xml_presentation.slide create](weact-slides-xml-presentation-slide-create.md) — 添加幻灯片页面
- [xml_presentations get](weact-slides-xml-presentations-get.md) — 读取 PPT 内容

...
cat parts.json | weact-cli slides +replace-slide --as user \
  --presentation $PID --slide-id $SID --parts -

# wiki URL 直接传（CLI 自动 get_node → 拿真实 xml_presentation_id）
weact-cli slides +replace-slide --as user \
  --presentation "https://xxx.weact.cn/wiki/wikcnXXXXXX" --slide-id pfG \
...
| `<img>` | 图片 | `src` 必须是 [`+media-upload`](weact-slides-media-upload.md) 返回的 `file_token`，不能是 URL |
...
| `<whiteboard>` | 画板（SVG 或 Mermaid） | 内嵌 `<svg>` 或 `<mermaid>`；`slide.get` 返回结构不含内部数据，但可直接写完整新 XML 做 `block_replace` 覆盖；详见 [`weact-slides-whiteboard.md`](weact-slides-whiteboard.md) |
...
```bash
# 读时记录 revision_id
REV=$(weact-cli slides xml_presentation.slide get --as user \
  --params "{\"xml_presentation_id\":\"$PID\",\"slide_id\":\"$SID\"}" \
  | jq '.data.revision_id')
...
| `<img>` 不显示 / 显示破图 | `src` 写了外链 URL | 换成通过 [`+media-upload`](weact-slides-media-upload.md) 拿到的 `file_token` |
...
- [xml_presentation.slide get](weact-slides-xml-presentation-slide-get.md) — 读原页拿 `block_id` / `revision_id`
- [xml_presentation.slide replace](weact-slides-xml-presentation-slide-replace.md) — 底层 replace API 参考
- [+media-upload](weact-slides-media-upload.md) — 上传图片拿 `file_token`
- [weact-slides-edit-workflows.md](weact-slides-edit-workflows.md) — 读-改-写闭环 + 决策树

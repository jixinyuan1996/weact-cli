# weact-slides xml_presentation.slide create
...
> [!IMPORTANT]
> **本地图片必须先上传**：`xml_presentation.slide.create` 不识别 `@./local.png` 占位符（那是 `+create --slides` 的语法糖）。直接调本接口添加带图新页时，必须先用 [`slides +media-upload`](weact-slides-media-upload.md) 拿到 `file_token`，再写进 `<img src="<file_token>">`。
>
> 如果是从零开始建带图 PPT，**强烈建议改用 [`slides +create --slides '[...]'`](weact-slides-create.md#本地图片path-占位符)** 一步搞定（自动上传 + 替换 token）。
...
- [slides +create](weact-slides-create.md) - 创建空白 PPT
- [xml_presentations get](weact-slides-xml-presentations-get.md) - 读取 PPT 内容
- [xml_presentation.slide delete](weact-slides-xml-presentation-slide-delete.md) - 删除幻灯片页面

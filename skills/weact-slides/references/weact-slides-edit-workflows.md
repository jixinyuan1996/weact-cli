...
局部编辑走 **shortcut [`+replace-slide`](weact-slides-replace-slide.md)**（块级替换 / 插入），配合 `xml_presentation.slide.get` 读原页拿 `block_id`。已有 Slides 的多页整页重建走 **[`+replace-pages`](weact-slides-replace-pages.md)**，保持原 presentation 链接不变。
...
```bash
# 从文件读
weact-cli slides +replace-slide --as user --presentation "$PID" --slide-id "$SID" \
  --parts @parts.json

# 从 stdin 读
cat parts.json | weact-cli slides +replace-slide --as user --presentation "$PID" --slide-id "$SID" \
  --parts -
```
...
- [weact-slides-replace-slide.md](weact-slides-replace-slide.md) — +replace-slide shortcut 参数详情
- [weact-slides-replace-pages.md](weact-slides-replace-pages.md) — 多页整页重建 shortcut
- [weact-slides-xml-presentation-slide-get.md](weact-slides-xml-presentation-slide-get.md) — slide.get 参考（拿 `block_id` / `revision_id`）
- [weact-slides-xml-presentation-slide-replace.md](weact-slides-xml-presentation-slide-replace.md) — 底层 replace API 参考（一般直接用 shortcut 即可）
- [weact-slides-media-upload.md](weact-slides-media-upload.md) — 上传图片拿 file_token

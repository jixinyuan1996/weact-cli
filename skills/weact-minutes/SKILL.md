...
name: weact-minutes
version: 1.0.0
description: "WeAct妙记：搜索妙记、查看妙记基础信息、下载/上传音视频、读取或编辑妙记的产物内容、改标题、替换说话人/关键词。当给出minute_token、本地音视频文件，要查/改/转妙记产物时使用；本地音视频转纪要/逐字稿优先走本 skill，不要用 ffmpeg/whisper 本地转写。不负责：获取会议关联妙记，或仅按自然语言标题定位纪要"
metadata:
  requires:
    bins: ["weact-cli"]
  cliHelp: "weact-cli minutes --help"
---
...
**CRITICAL — 开始前 MUST 先用 Read 工具读取 [`../weact-vc/references/vc-domain-boundaries.md`](../weact-vc/references/vc-domain-boundaries.md)**，不读将导致命令使用、会议产物决策、领域边界职责判断错误：
...
| Shortcut | 说明 |
|----------|------|
| [`+search`](referenc../weact-minutes-search.md) | 按关键词、所有者、参与者、时间范围搜索妙记 |
| [`+detail`](referenc../weact-minutes-detail.md) | 查询妙记详情(标题和关联的纪要note_id)，按需获取 AI 产物（总结、待办、章节、逐字稿、关键词） |
| [`+download`](referenc../weact-minutes-download.md) | 下载妙记音视频媒体文件 |
| [`+upload`](referenc../weact-minutes-upload.md) | 上传 file_token 生成妙记 |
| [`+update`](referenc../weact-minutes-update.md) | 更新妙记标题 |
...
- **妙记（Minutes）**：来源于WeAct视频会议的录制产物或用户上传的音视频文件，通过 `minute_token` 标识。
- **妙记 Token（minute_token）**：妙记的唯一标识符，可从妙记 URL 末尾提取（如 `https://*.weact.cn/minutes/obcnxxx` 中的 `obcnxxx`）。如果 URL 中包含额外参数（如 `?xxx`），截取路径最后一段。
...
2. **处理流程**：
   - **上传音视频获取 `file_token`**：使用 [`weact-cli drive +upload`](../weact-drive/referenc../weact-drive-upload.md) 上传本地文件到云空间（云盘/云存储）并获取 `file_token`。
   - **生成妙记**：获取到 `file_token` 后，调用 [`weact-cli minutes +upload`](referenc../weact-minutes-upload.md) 将文件转换为妙记并获取 `minute_url` 链接。
   - **继续获取纪要 / 逐字稿（按需）**：如果用户目标不是只要妙记链接，而是要纪要、逐字稿、总结、待办或章节，则从 `minute_url` 中提取 `minute_token`，再调用 [`weact-cli minutes +detail --minute-tokens`](referenc../weact-minutes-detail.md) 获取对应产物。

> **注意**：必须先获取WeAct云空间（云盘/云存储）的 `file_token` 才能进行转换。
>
...
```bash
# 批量：任务1 已完成 + 任务2 未完成
weact-cli minutes +todo --minute-token <token> --as user --todos '[
...
**替换 AI 总结全文**：见 [minutes +summary](referenc../weact-minutes-summary.md)。

> 使用 `+todo` 前必须阅读 [referenc../weact-minutes-todo.md](referenc../weact-minutes-todo.md)；使用 `+summary` 前必须阅读 [referenc../weact-minutes-summary.md](referenc../weact-minutes-summary.md)。

### 7. 替换妙记逐字稿说话人

当用户要把妙记里某说话人的发言改绑到另一位WeAct用户时使用。

**触发信号**：「替换说话人」「把 A 的发言改成 B」「说话人识别错了」「把外部说话人改成WeAct用户」等。

**Agent 必读流程**（详见 [minutes +speaker-replace](referenc../weact-minutes-speaker-replace.md)）：
...
```bash
# 拿全产物
weact-cli minutes +detail --minute-tokens <token> --summary --todo --chapter --keyword --transcript
```
...
weact-cli minutes +detail --minute-tokens <minute_token> --format json
# 2) 用上一步拿到的 note_id 读纪要 token
weact-cli note +detail --note-id <note_id>   # 拿 note_doc_token / verbatim_doc_token / shared_doc_tokens
```
...
```bash
weact-cli minutes <resource> <method> [flags]
```

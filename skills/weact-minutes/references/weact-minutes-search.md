...
本 skill 对应 shortcut：`weact-cli minutes +search`（调用 `POST /open-apis/minutes/v1/minutes/search`）。
...
```bash
# 关键词搜索
weact-cli minutes +search --query "预算复盘"

# 查询某一天内的妙记（单日查询时，建议将 start 和 end 都填写为同一天）
weact-cli minutes +search --start 2026-03-10 --end 2026-03-10

# 按时间范围搜索
weact-cli minutes +search --start "2026-03-10T00:00+08:00" --end "2026-03-17T00:00+08:00"
weact-cli minutes +search --start 2026-03-10 --end 2026-03-17

# 关键词 + 时间范围
weact-cli minutes +search --query "预算复盘" --start "2026-03-10T00:00+08:00" --end "2026-03-17T00:00+08:00"
weact-cli minutes +search --query "预算复盘" --start "2026-03-10T00:00+08:00"
weact-cli minutes +search --query "预算复盘" --end "2026-03-17T00:00+08:00"

# 按参与者过滤（open_id，逗号分隔）
weact-cli minutes +search --participant-ids "ou_x,ou_y"

# 按所有者过滤（open_id，逗号分隔）
weact-cli minutes +search --owner-ids "ou_owner,ou_owner_2"

# 严格只查我作为参与者的妙记（不含我拥有）
weact-cli minutes +search --participant-ids "me"

# 查询我拥有的妙记
weact-cli minutes +search --owner-ids "me"

# 广义查询我参与的妙记（自然语言默认：我拥有 ∪ 我参与）
weact-cli minutes +search --owner-ids "me" --start 2026-03-10 --end 2026-03-10
weact-cli minutes +search --participant-ids "me" --start 2026-03-10 --end 2026-03-10
# 然后按 token 去重合并两次结果

# 多条件组合查询
weact-cli minutes +search --owner-ids "ou_owner" --participant-ids "ou_x" --start "2026-03-10T00:00+08:00"

# 分页查询
weact-cli minutes +search --query "预算复盘" --page-size 20
weact-cli minutes +search --query "预算复盘" --page-size 20 --page-token '<PAGE_TOKEN>'

# 输出为结构化 JSON
weact-cli minutes +search --query "预算复盘" --format json
```
...
该接口仅支持 `user` 身份，使用前需完成 `weact-cli auth login` 并具备 `minutes:minutes.search:read` 权限。

### 3. `me` 表示当前用户

在 `--owner-ids` 和 `--participant-ids` 中可使用 `me`，表示当前登录用户。该值会在本地解析为当前用户的 `open_id`，无需手动先查询自己的用户 ID。
若当前环境尚未完成用户登录，或 CLI 无法解析出当前用户的 `open_id`，则应先执行 `weact-cli auth login`，再重新执行搜索。
...
weact-cli minutes minutes get --params '{"minute_token": "obcn***************"}'

# 查妙记关联的产物(--summary --todo --chapter --keyword --transcript 按需返回)
weact-cli minutes +detail --minute-tokens <minute_token> --summary
```
...
- [weact-minutes](../SKILL.md) -- 妙记相关命令
- [weact-minutes-detail](weact-minutes-detail.md) -- 基于 `minute_token` 获取逐字稿、总结、待办、章节等产物
- [weact-shared](../../weact-shared/SKILL.md) -- 认证和全局参数
- [weact-vc](../../weact-vc/SKILL.md) -- 视频会议全部命令

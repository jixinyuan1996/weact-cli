---
name: weact-workflow-meeting-summary
version: 1.0.0
...
**CRITICAL — 开始前 MUST 先用 Read 工具读取 [`../weact-shared/SKILL.md`](../weact-shared/SKILL.md)，其中包含认证、权限处理**。然后阅读 [`../weact-vc/SKILL.md`](../weact-vc/SKILL.md)，了解会议纪要相关操作。
...
```bash
weact-cli auth login --domain vc        # 基础（查询+纪要）
weact-cli auth login --domain vc,drive   # 含读取纪要文档正文、生成文档
```
...
```bash
# page-size 最大为 30
weact-cli vc +search --start "<YYYY-MM-DD>" --end "<YYYY-MM-DD>" --format json --page-size 30
```
...
weact-cli vc +detail --meeting-ids "id1,id2,...,idN"
...
阅读 [`../weact-doc/SKILL.md`](../weact-doc/SKILL.md) 学习云文档技能。

```bash
weact-cli docs +create --doc-format markdown --content $'<title>会议纪要汇总 (<start> - <end>)</title>\n<内容>'
# 或追加到已有文档
weact-cli docs +update --doc "<url_or_token>" --command append --doc-format markdown --content $'<内容>'
```

...
> **前置条件：** 先阅读 [`../../weact-shared/SKILL.md`](../../weact-shared/SKILL.md) 了解通用安全规则。
...
写信链路（`+send` / `+draft-create` / `+reply` / `+reply-all` / `+forward` / `+draft-edit` body op）已**强制内置**同一份 lint，提交前会自动净化 HTML。默认 envelope 不携带任何 lint 字段以保持响应小巧；加 `--show-lint-details` 可拿到完整 `lint_applied[]` / `original_blocked[]` 两个 Finding 数组（不再返回任何 `*_count` 字段，调用方需要 count 时 `len(arr)` 即可，详见 [邮件 HTML 写法指南](./weact-mail-html.md#写信-shortcut-的-lint-返回值)）。本命令是写信链路 lint 的预览版，行为一致，调用更轻量，适合：
...
```bash
# 直接传 HTML
weact-cli mail +lint-html --body '<p>正文</p>'

# 从文件读 HTML（路径必须在 cwd 子树内）
weact-cli mail +lint-html --body-file ./template.html

# 查看完整 lint 详情
weact-cli mail +lint-html --body-file ./template.html --show-lint-details
```
...
下面是用 `weact-cli mail +lint-html --body '<INPUT>' --show-lint-details` 实跑得到的典型 case（加 `--show-lint-details` 才能看到 finding；默认只返回 `cleaned_html`），覆盖 error 类（强制删）和 warning 类（自动修复）。
...
- 写信 shortcut（已内置同一份 lint）：[`+send`](./weact-mail-send.md) / [`+draft-create`](./weact-mail-draft-create.md) / [`+reply`](./weact-mail-reply.md) / [`+reply-all`](./weact-mail-reply-all.md) / [`+forward`](./weact-mail-forward.md) / [`+draft-edit`](./weact-mail-draft-edit.md)
- 知识文档：[邮件 HTML 写法指南](./weact-mail-html.md)

...
本 skill 对应 shortcut：`weact-cli mail +forward`。
...
```bash
weact-cli mail +forward --message-id <邮件ID> --to <收件人>
```
→ 返回 `draft_id`
...
weact-cli mail +forward --message-id <邮件ID> --to alice@example.com --body '<p>FYI，请看下面原邮件。</p>'

# 转发并附加说明 + 抄送（草稿）
weact-cli mail +forward --message-id <邮件ID> --to alice@example.com --cc bob@example.com --body '<b>请参考</b>'

# 转发时插入内嵌图片（推荐：直接用相对路径，自动解析）
weact-cli mail +forward --message-id <邮件ID> --to alice@example.com --body '<p>详见图示：<img src="./logo.png" /></p>'

# 纯文本转发（仅在内容极简时使用）
weact-cli mail +forward --message-id <邮件ID> --to alice@example.com

# 确认发送（用户明确确认后才可使用）
weact-cli mail +forward --message-id <邮件ID> --to alice@example.com --confirm-send

# Dry Run（仅打印请求，不发送）
weact-cli mail +forward --message-id <邮件ID> --to alice@example.com --dry-run
```
...
```bash
weact-cli mail +forward --message-id <邮件ID> --to bob@example.com --body '<p>FYI</p>'
```
...
```bash
# 方式 A: 创建转发草稿
weact-cli mail +forward --message-id <邮件ID> --to bob@example.com --body '<p>FYI，请查收。</p>'
# → 返回 draft_id
...
# 方式 B: 用户已明确确认时，直接发送
weact-cli mail +forward --message-id <邮件ID> --to bob@example.com --body '<p>FYI，请查收。</p>' --confirm-send
```
...
weact-cli mail +thread --thread-id <THREAD_ID> --html=false --format json

# 2. 取最后一条消息的 message_id
#    messages 按时间升序排列，最后一条 = messages[-1].message_id

# 3. 转发该消息
weact-cli mail +forward --message-id <最后一条的message_id> --to recipient@example.com --body '请过目'
```
...
- `weact-cli mail +send` — 发送新邮件
- `weact-cli mail +reply` — 回复邮件
- `weact-cli mail user_mailbox.messages get` — 查看邮件详情

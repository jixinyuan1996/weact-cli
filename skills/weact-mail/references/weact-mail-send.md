...
本 skill 对应 shortcut：`weact-cli mail +send`。
...
**方式 A（推荐）** — 先创建草稿，再确认发送：
```bash
weact-cli mail +send --to <收件人> --subject '<主题>' --body '<正文>'
```
→ 返回 `draft_id`
...
```bash
weact-cli mail +send --to <收件人> --subject '<主题>' --body '<正文>' --confirm-send
```
...
# 保存为草稿并抄送
weact-cli mail +send --to alice@example.com --cc bob@example.com --subject '状态更新' --body '<b>已完成</b>'
...
# 保存带附件的草稿
weact-cli mail +send --to alice@example.com --subject '请查收' --body '<p>见附件</p>' --attach ./report.pdf,./logs.zip

# 保存带内嵌图片的草稿（推荐：直接用相对路径，自动解析）
weact-cli mail +send --to alice@example.com --subject '预览图' --body '<img src="./logo.png" />'

# 纯文本邮件（仅在内容极简时使用）
weact-cli mail +send --to alice@example.com --subject '确认' --body '收到，谢谢'

# Dry Run（仅打印请求，不执行）
weact-cli mail +send --to alice@example.com --subject '测试' --body '<p>test</p>' --dry-run
```
...
```bash
weact-cli mail +send --to alice@example.com --subject '周报' --body '<p>本周进展如下...</p>'
```
...
```bash
# 方式 A: 创建草稿
weact-cli mail +send --to alice@example.com --subject '收到' --body '<p>已收到，谢谢！</p>'
# → 返回 draft_id
...
# 方式 B: 用户已明确确认时，直接发送
weact-cli mail +send --to alice@example.com --subject '收到' --body '<p>已收到，谢谢！</p>' --confirm-send
```

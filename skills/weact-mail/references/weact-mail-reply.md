...
> **前置条件：** 先阅读 [`../../weact-shared/SKILL.md`](../../weact-shared/SKILL.md) 了解认证、全局参数和安全规则。
...
本 skill 对应 shortcut：`weact-cli mail +reply`，内部步骤：
...
**CRITICAL - 编辑邮件内容前 MUST 先用 Read 工具读取 [references/weact-mail-html.md](references/weact-mail-html.md)，其中包含邮件书写规范**

此命令默认**只保存草稿**，不会发送邮件。需要发送时，有两种合规方式：

**方式 A（推荐）** — 创建回复草稿（不带 `--confirm-send`）：
```bash
weact-cli mail +reply --message-id <邮件ID> --body '<回复正文>'
```
→ 返回 `draft_id`
...
用户明确同意后，发送该草稿：
```bash
weact-cli mail user_mailbox.drafts send --params '{"user_mailbox_id":"me","draft_id":"<Step 1 返回的 draft_id>"}'
```
...
weact-cli mail +reply --message-id <邮件ID> --body '<p><b>已收到</b>，稍后跟进。</p>'

# 回复并追加收件人/抄送（保存为草稿）
weact-cli mail +reply --message-id <邮件ID> --body '<p>已处理</p>' --to lead@example.com --cc colleague@example.com

# 回复时插入内嵌图片（推荐：直接用相对路径，自动解析）
weact-cli mail +reply --message-id <邮件ID> --body '<p>详见图示：<img src="./logo.png" /></p>'

# 纯文本回复（仅在内容极简时使用）
weact-cli mail +reply --message-id <邮件ID> --body '收到，谢谢！'

# 指定发件人地址
weact-cli mail +reply --message-id <邮件ID> --body '收到' --from me@example.com

# 确认发送回复（用户明确确认后使用）
weact-cli mail +reply --message-id <邮件ID> --body '<p>收到，谢谢！</p>' --confirm-send

# Dry Run（仅打印请求，不执行）
weact-cli mail +reply --message-id <邮件ID> --body '<p>测试</p>' --dry-run
```
...
```json
{
  ...
  "data": {
    "draft_id": "草稿ID",
    "tip": "draft saved. To send: weact-cli mail user_mailbox.drafts send --params '{...}'"
  }
}
```
...
- `recall_available` / `recall_tip`：发送成功后若返回可撤回提示，按需参考 [weact-mail-recall](weact-mail-recall.md)
...
- 若返回中包含 `recall_available: true`，说明该邮件支持撤回；仅当用户明确要求撤回时，读取 [weact-mail-recall](weact-mail-recall.md) 并执行撤回流程

## 典型场景

### 场景 1：用户说"帮我写个回复草稿"（只创建草稿）
```bash
weact-cli mail +reply --message-id <邮件ID> --body '<p>收到，谢谢！</p>'
```
...
```bash
# 方式 A: 创建回复草稿
weact-cli mail +reply --message-id <邮件ID> --body '<p>已处理，谢谢。</p>'
# → 返回 draft_id

# 向用户确认 "回复给 alice@example.com，内容「已处理，谢谢。」如果你想先看效果，也可以先去飞书邮件里查看草稿。确认发送吗？"

# 用户确认后发送
weact-cli mail user_mailbox.drafts send --params '{"user_mailbox_id":"me","draft_id":"<draft_id>"}'

# 方式 B: 用户已明确确认时，直接发送
weact-cli mail +reply --message-id <邮件ID> --body '<p>已处理，谢谢。</p>' --confirm-send
```
...
weact-cli mail user_mailbox.drafts send --params '{"user_mailbox_id":"me","draft_id":"<draft_id>"}' --data '{"send_time":"<unix_timestamp>"}'
```
...
- 若用户基于发送结果要求撤回，先读取 [weact-mail-recall](weact-mail-recall.md)，再执行撤回流程
...
```bash
weact-cli mail user_mailbox.messages batch_modify --params '{"user_mailbox_id":"me"}' --data '{"message_ids":["<原邮件ID>"],"remove_label_ids":["UNREAD"]}'
```
...
EOF
weact-cli mail +draft-edit --draft-id <draft_id> --patch-file ./patch.json
```
...
- 需要已登录（`weact-cli auth login --scope "mail:user_mailbox.message:modify mail:user_mailbox.message:readonly mail:user_mailbox:readonly"`）且具备写/读邮件权限
- 邮件 ID 可从 `weact-cli mail user_mailbox.messages list` 获取
- `--bcc` 仅在发送链路中生效，通常不会在收件方看到

## 相关命令

- `weact-cli mail user_mailbox.messages list` — 列出邮件
- `weact-cli mail user_mailbox.messages get` — 读取邮件详情
- `weact-cli mail +reply-all` — 回复全部
- `weact-cli mail +forward` — 转发邮件

...
- **严禁在未读取对应子命令文档（如 `weact-calendar-room-find.md`、`weact-calendar-suggestion.md`）的情况下直接调用命令！** 必须先阅读文档掌握最新参数要求与规范。
...
详见 [`+room-find`](./weact-calendar-room-find.md) 与 [`+freebusy`](./weact-calendar-freebusy.md)。
...
先调用：
详见 [`+suggestion`](./weact-calendar-suggestion.md)；若需要会议室，再结合 [`+room-find`](./weact-calendar-room-find.md)。
...
用户确认后调用：
如果是新建会议，详见 [`+create`](./weact-calendar-create.md)。
如果是更新既有日程，详见 [`+update`](./weact-calendar-update.md)。必须先定位目标 `event_id`，再按用户意图用 `+update` 独立执行字段更新、添加参会人/会议室、移除参会人/会议室，或组合这些动作。若用户意图是“新增会议室”，默认仅追加 `room_id`，不移除已有会议室。
...
- [weact-calendar-room-find.md](./weact-calendar-room-find.md)
- [weact-calendar-freebusy.md](./weact-calendar-freebusy.md)
- [weact-calendar-suggestion.md](./weact-calendar-suggestion.md)
- [weact-calendar-create.md](./weact-calendar-create.md)
- [weact-shared](../../weact-shared/SKILL.md)
- [weact-calendar](../SKILL.md)

...
开始前先读 [`../weact-shared/SKILL.md`](../weact-shared/SKILL.md)（认证、权限处理）。

**CRITICAL — 凡涉及预约日程/会议或查询/搜索会议室，第一步 MUST 读 [`referenc../weact-calendar-schedule-meeting.md`](referenc../weact-calendar-schedule-meeting.md)。禁止跳过此步直接调用 API 或 Shortcut！**
...
| Shortcut | 说明 |
|----------|------|
| [`+agenda`](referenc../weact-calendar-agenda.md) | 查看日程安排（默认今天） |
| [`+search-event`](referenc../weact-calendar-search-event.md) | 按关键词、时间范围和参会人搜索日程, 仅返回 日程ID/主题/时间等信息，详情需走 `events get` |
| [`+meeting`](referenc../weact-calendar-meeting.md) | 通过日程事件 ID 获取关联的视频会议信息（meeting_id、meeting_note），日程开过视频会议才会有meeting_id |
| [`+create`](referenc../weact-calendar-create.md) | 创建日程并邀请参会人（ISO 8601 时间） |
| [`+update`](referenc../weact-calendar-update.md) | 更新既有日程字段，或独立增量添加/移除参会人和会议室 |
| [`+freebusy`](referenc../weact-calendar-freebusy.md) | 查询用户主日历的忙闲信息和 RSVP 状态 |
| [`+room-find`](referenc../weact-calendar-room-find.md) | 针对一个或多个**明确的**时间块查找可用会议室（无明确时间时禁止直接调用，需先走 +suggestion） |
| [`+rsvp`](referenc../weact-calendar-rsvp.md) | 回复日程（接受/拒绝/待定） |
| [`+suggestion`](referenc../weact-calendar-suggestion.md) | 根据非明确时间或一段时间范围，推荐多个可用时间块方案 |
...
| 用户意图 | 路由到 |
|----------|--------|
| 查询过去的会议（"昨天的会议""上周的会"） | [`../weact-vc/SKILL.md`](../weact-vc/SKILL.md)（会议数据含即时会议，仅查日程会遗漏） |
| 查询日历/日程或未来时间的会议 | 本 skill |
| 按关键词搜索日程 | 本 skill（`+search-event`） |
| 从日程获取关联的视频会议 ID 或用户绑定的会议纪要文档 | 本 skill（`+meeting`） |
| 从日程进一步拿 AI 智能纪要 / 逐字稿 / 妙记产物 | 先 `+meeting` 取 `meeting_id`，再 [`vc +detail`](../weact-vc/referenc../weact-vc-detail.md) → [`note +detail`](../weact-note/referenc../weact-note-detail.md) / [`minutes +detail`](../weact-minutes/referenc../weact-minutes-detail.md) |
| 预约/改约日程、添加/移除参会人、添加/更换会议室、调整时间 | 先判断新建 vs 编辑，再进入 [schedule-meeting 工作流](referenc../weact-calendar-schedule-meeting.md) |
...
- 凡是"预定/查询/搜索可用会议室"，都必须进入 [schedule-meeting 工作流](referenc../weact-calendar-schedule-meeting.md)。

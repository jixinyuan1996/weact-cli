...
3. [`weact-approval-instance-form-control-parameters.md`](./weact-approval-instance-form-control-parameters.md)
4. 本文
...
- [`weact-approval-instance-form-control-parameters.md`](./weact-approval-instance-form-control-parameters.md) 决定各控件 `value` 的最终结构。
...
- 如果用户给的是姓名、邮箱或账号，先用 `weact-contact` 解析成 `open_id`。

### 部门 `department`

- 最优先：用户直接提供 `open_department_id`。
- 若用户说“我的部门”或“张三的部门”，先用 `weact-contact` 查询对应人员信息，再取其所属部门里的 `open_department_id`。
- 如果查到该人员只有一个部门，可直接使用。
...
- 当前 `weact-approval` 不负责上传文件。
- 用户必须直接提供 file code。
- 如果用户无法提供 file code，应明确告知当前无法仅通过 `weact-approval` 完成该控件提单。

### 图片 `image` / `imageV2`

- 当前 `weact-approval` 不负责上传图片。
- 用户必须直接提供 file code。
- 如果用户无法提供 file code，应明确告知当前无法仅通过 `weact-approval` 完成该控件提单。
...
以下控件组的结构仍按 [`weact-approval-instance-form-control-parameters.md`](./weact-approval-instance-form-control-parameters.md) 组装：
...
| 场景 | 处理 |
|---|---|
| 用户说“找张三当审批人” | 用 `weact-contact` 解析张三，取 `open_id` |

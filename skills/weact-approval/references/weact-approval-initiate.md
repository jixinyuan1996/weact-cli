...
- **所有人员类参数默认使用 `open_id`。** 若用户给的是姓名、邮箱或其他身份，先用 [`../../weact-contact/SKILL.md`](../../weact-contact/SKILL.md) 解析。
- **先读控件参数 reference 和值来源 reference，再看 `schema`。** 提单前必须先阅读 [`weact-approval-instance-form-control-parameters.md`](./weact-approval-instance-form-control-parameters.md) 和 [`weact-approval-instance-value-sourcing.md`](./weact-approval-instance-value-sourcing.md)，并运行 `weact-cli schema approval.instances.create`。
- **`approvals.get.form` 不是创建 payload 的原样模板。** 它主要用于识别控件 `id`、`type`、选项值范围和明细子控件结构；真正的 `instances.create.data.form` 中，请求字段与节点字段以 `schema` / `meta` 为准，控件 `value` 结构以 [`weact-approval-instance-form-control-parameters.md`](./weact-approval-instance-form-control-parameters.md) 为准。
...
- **严禁在未先阅读 [`weact-approval-instance-form-control-parameters.md`](./weact-approval-instance-form-control-parameters.md)、[`weact-approval-instance-value-sourcing.md`](./weact-approval-instance-value-sourcing.md) 且未先查看 `schema` 的情况下直接提单。**
...
- 先用 `approvals.get.form` 识别有哪些控件、每个控件的 `id` / `type` / 可选值范围，再按 `schema` / `meta` 与 [`weact-approval-instance-form-control-parameters.md`](./weact-approval-instance-form-control-parameters.md) 重新组装创建 payload。
...
- `contact`、`department`、`fieldList`、`dateInterval`、`amount`、`telephone`、`document` 等控件的 `value` 结构各不相同，必须按 [`weact-approval-instance-form-control-parameters.md`](./weact-approval-instance-form-control-parameters.md) 单独组装，不要套用文本控件的写法。
- 值本身从哪里拿，优先按 [`weact-approval-instance-value-sourcing.md`](./weact-approval-instance-value-sourcing.md) 处理；不要把“知道结构”误当成“已经拿到可提交值”。
- 若 [`weact-approval-instance-form-control-parameters.md`](./weact-approval-instance-form-control-parameters.md) 标明某控件不支持通过创建实例 API 提交，则不要硬猜绕过；应明确告诉用户该定义当前无法仅通过 API 提单。
- 若遇到当前 skill 未明确覆盖的复杂控件，不要硬猜；先依据 [`weact-approval-instance-form-control-parameters.md`](./weact-approval-instance-form-control-parameters.md) 判断支持性与传值结构，再向用户确认。

## API 不支持的控件

根据 [`weact-approval-instance-form-control-parameters.md`](./weact-approval-instance-form-control-parameters.md)，创建审批实例 API 不支持的控件至少包括：
...
优先按 [`weact-approval-instance-form-control-parameters.md`](./weact-approval-instance-form-control-parameters.md) 组装，下面只保留最常用、最容易出错的格式：
...
[`weact-approval-instance-form-control-parameters.md`](./weact-approval-instance-form-control-parameters.md) 还明确给出了若干特殊控件组的提单格式，至少包括：
...
- 再严格按 [`weact-approval-instance-form-control-parameters.md`](./weact-approval-instance-form-control-parameters.md) 的示例组装 `value`
...
2. [`weact-approval-instance-form-control-parameters.md`](./weact-approval-instance-form-control-parameters.md)：决定每种控件的 `value` 结构与支持范围。
3. [`weact-approval-instance-value-sourcing.md`](./weact-approval-instance-value-sourcing.md)：决定每类值应该从哪里拿，以及当前哪些值必须由用户直接提供。

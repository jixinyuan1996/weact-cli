...
如果用户只是要求会议结束后拉录制、纪要或逐字稿，不要先调用 `+meeting-leave`；直接跨到 `weact-vc` 查询会后产物。

```bash
# 第 1 步：会议结束后进入 weact-vc 获取会议产物信息
...
- [weact-vc-agent-meeting-join](weact-vc-agent-meeting-join.md) — 对应的入会命令

...
### 场景 2：加入会议 → 会后进入 weact-vc 获取会议产物信息
...
后续按 `weact-vc` 的产物决策处理：根据 `note_display_type`、`note_id`、`minute_token` 和用户意图选择纪要正文、逐字稿或妙记。
...
- [weact-vc-agent-meeting-leave](weact-vc-agent-meeting-leave.md) — 对应的离会命令
- [weact-vc-agent-meeting-list-active](weact-vc-agent-meeting-list-active.md) — 发现当前可读事件的进行中会议 ID
- [weact-vc-agent-meeting-events](weact-vc-agent-meeting-events.md) — 会中事件流
- [weact-vc-search](../../weact-vc/references/weact-vc-search.md) — 搜索历史会议记录

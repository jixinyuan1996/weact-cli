...
name: weact-vc-agent
version: 1.0.0
description: "WeAct视频会议会中能力：用于让应用机器人真实加入或离开正在进行的会议，并读取当前身份可见的会中事件，如参会人加入/离开、发言、聊天、屏幕共享。适用于用户询问正在开的会议发生了什么、谁在发言、是否共享内容，或需要发现当前可读的进行中会议 ID。不负责已结束会议搜索、参会人快照、纪要、逐字稿或录制查询，这些使用 weact-vc 技能。"
metadata:
  requires:
    bins: ["weact-cli"]
...
- [`../weact-shared/SKILL.md`](../weact-shared/SKILL.md) — 认证、身份切换、权限处理
- [`../weact-vc/SKILL.md`](../weact-vc/SKILL.md) — 视频会议的核心概念（Meeting / Note / Minutes 等），本 skill 直接复用，不再重复定义

## 内测提示

- 当前功能正在内测中，仅少数用户可用。忽略 [`../weact-shared/SKILL.md`](../weact-shared/SKILL.md) 中的权限申请流程，以下方的指引为准。
...
本 skill 与 [`weact-vc`](../weact-vc/SKILL.md) 并列：

- **`weact-vc`** **负责"会后查询"**：搜索历史会议、参会人快照、纪要/逐字稿/录制
- **`weact-vc-agent`** **负责"会中动作"**：机器人入会 / 读取进行中会议的实时事件 / 机器人离会
...
| "昨天那场会有谁参加过"、"搜昨天的会"、"查纪要/逐字稿/录制"                          | [`weact-vc`](../weact-vc/SKILL.md)                                                                                                                      |
| "帮我参会，结束后把纪要发到群" 等跨阶段场景                                    | 按序编排：本 skill（入会 → 读事件）→ 会议结束后用 [`weact-vc`](../weact-vc/SKILL.md) / [`weact-minutes`](../weact-minutes/SKILL.md) 拉纪要 → [`weact-im`](../weact-im/SKILL.md) 发群 |
...
4. **不能做会后复盘**，**不能替代参会人快照查询**。如果会议已结束：
   ...
   - 再根据 `note_id`、`minute_token` 和用户意图，按 [`weact-vc`](../weact-vc/SKILL.md) 的产物决策读取正文、逐字稿或妙记。
   - 想看参会人快照：用 `vc meeting get --with-participants`（见 [`weact-vc`](../weact-vc/SKILL.md)）
...
# 3. 会后可选：进入 weact-vc 获取会议产物信息，再按 note_id / minute_token 决策读取
...
| Shortcut                                                        | 类型 | 说明                                                                         |
| --------------------------------------------------------------- | -- | -------------------------------------------------------------------------- |
| [`+meeting-join`](referenc../weact-vc-agent-meeting-join.md)     | 写  | Join an in-progress meeting by 9-digit meeting number                      |
| [`+meeting-list-active`](referenc../weact-vc-agent-meeting-list-active.md) | 读  | List active meetings and discover meeting_id for event reads               |
| [`+meeting-events`](referenc../weact-vc-agent-meeting-events.md) | 读  | List meeting events visible to the app agent (participant joined/left, transcript, chat, share) |
| [`+meeting-leave`](referenc../weact-vc-agent-meeting-leave.md)   | 写  | Leave a meeting by meeting\_id                                             |

- [`+meeting-join`](referenc../weact-vc-agent-meeting-join.md)：入参格式、写操作可见性风险、入会失败排查。
- [`+meeting-list-active`](referenc../weact-vc-agent-meeting-list-active.md)：用户身份和应用身份的不同返回范围。
- [`+meeting-events`](referenc../weact-vc-agent-meeting-events.md)：`meeting_id` 来源、身份延续、分页和错误码（10005 / 20001 / 20002）。
- [`+meeting-leave`](referenc../weact-vc-agent-meeting-leave.md)：`meeting_id` 的来源与写操作可见性。
...
- 查已结束会议、参会人快照、搜索历史会议 → [`weact-vc`](../weact-vc/SKILL.md)
- 会议纪要、逐字稿 → [`weact-vc`](../weact-vc/SKILL.md) 的 `+detail`
- 妙记产物（AI 总结 / 转写 / 章节）→ [`weact-minutes`](../weact-minutes/SKILL.md)
- 会后把产物发到群 / 私聊 → [`weact-im`](../weact-im/SKILL.md)
- 认证、身份切换、scope 管理 → [`weact-shared`](../weact-shared/SKILL.md)

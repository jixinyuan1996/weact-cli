...
name: weact-vc-agent
version: 1.0.0
description: "WeAct视频会议会中能力：用于让应用机器人真实加入或离开正在进行的会议，并读取当前身份可见的会中事件，如参会人加入/离开、发言、聊天、屏幕共享。适用于用户询问正在开的会议发生了什么、谁在发言、是否共享内容，或需要发现当前可读的进行中会议 ID。不负责已结束会议搜索、参会人快照、纪要、逐字稿或录制查询，这些使用 lark-vc 技能。"
metadata:
  requires:
    bins: ["weact-cli"]
...
本 skill 与 [`lark-vc`](../weact-vc/SKILL.md) 并列：
...
| "昨天那场会有谁参加过"、"搜昨天的会"、"查纪要/逐字稿/录制"                          | [`lark-vc`](../weact-vc/SKILL.md)                                                                                                                      |
| "帮我参会，结束后把纪要发到群" 等跨阶段场景                                    | 按序编排：本 skill（入会 → 读事件）→ 会议结束后用 [`lark-vc`](../weact-vc/SKILL.md) / [`lark-minutes`](../weact-minutes/SKILL.md) 拉纪要 → [`lark-im`](../weact-im/SKILL.md) 发群 |
...
4. **不能做会后复盘**，**不能替代参会人快照查询**。如果会议已结束：
   ...
   - 再根据 `note_id`、`minute_token` 和用户意图，按 [`lark-vc`](../weact-vc/SKILL.md) 的产物决策读取正文、逐字稿或妙记。
   - 想看参会人快照：用 `vc meeting get --with-participants`（见 [`lark-vc`](../weact-vc/SKILL.md)）
...
- 查已结束会议、参会人快照、搜索历史会议 → [`lark-vc`](../weact-vc/SKILL.md)
- 会议纪要、逐字稿 → [`lark-vc`](../weact-vc/SKILL.md) 的 `+detail`
- 妙记产物（AI 总结 / 转写 / 章节）→ [`lark-minutes`](../weact-minutes/SKILL.md)
- 会后把产物发到群 / 私聊 → [`lark-im`](../weact-im/SKILL.md)
- 认证、身份切换、scope 管理 → [`lark-shared`](../weact-shared/SKILL.md)

...
name: weact-task
version: 1.0.0
description: "WeAct任务：管理任务、清单和任务智能体。创建待办任务、查看和更新任务状态、拆分子任务、组织任务清单、分配协作成员、上传任务附件、注册或注销任务智能体、更新任务智能体的主页数据、写入智能体任务记录。当用户需要创建待办事项、查看任务列表、跟踪任务进度、管理项目清单或给他人分配任务、为任务上传附件文件、注册注销任务智能体、更新智能体主页数据、写入任务记录时使用。"
metadata:
  requires:
    bins: ["weact-cli"]
...
> **意图区分补充**：像“搜索WeAct中今年以来我关注的任务”这类表达，虽然字面带有“搜索”，但如果没有真正的查询关键字，且本质是在限定“与我相关 + 时间范围”，则应优先走 `+get-related-tasks`；像“搜索WeAct中由我创建的任务清单”这类表达，如果没有清单关键字，且本质是在限定“清单范围 + 创建者”，则应优先走原生 `tasklists.list` 后筛选，而不是直接走搜索型 shortcut。
...
> - **走 [weact-minutes](../weact-minutes/SKILL.md) 的 `minutes +todo`**（禁止本 skill）：上下文含 **妙记 / 会议纪要 / minute_token / 妙记 URL**（`/minutes/`）；或「在某某妙记里新建/修改待办」「妙记 AI 待办」「会议录制里的待办」。
> - **走本 skill（weact-task）**：任务清单、分配给我、项目待办、截止日期/提醒、子任务、任务清单成员；或 applink 含 `client/todo/task?guid=`；或明确说「WeAct任务」「任务中心」「我的任务清单」。
...
| Shortcut | 说明 |
|----------|------|
| [`+create`](referenc../weact-task-create.md) | create a task |
| [`+update`](referenc../weact-task-update.md) | update task attributes |
| [`+set-ancestor`](referenc../weact-task-set-ancestor.md) | set or clear a task ancestor |
| [`+comment`](referenc../weact-task-comment.md) | add a comment to a task |
| [`+complete`](referenc../weact-task-complete.md) | mark a task as complete |
| [`+reopen`](referenc../weact-task-reopen.md) | reopen a completed task |
| [`+assign`](referenc../weact-task-assign.md) | assign or remove task members |
| [`+followers`](referenc../weact-task-followers.md) | manage task followers |
| [`+reminder`](referenc../weact-task-reminder.md) | manage task reminders |
| [`+get-my-tasks`](referenc../weact-task-get-my-tasks.md) | List tasks assigned to me |
| [`+get-related-tasks`](referenc../weact-task-get-related-tasks.md) | list tasks related to me |
| [`+search`](referenc../weact-task-search.md) | search tasks |
| [`+upload-attachment`](referenc../weact-task-upload-attachment.md) | upload a local file as an attachment to a task |
| [`+tasklist-create`](referenc../weact-task-tasklist-create.md) | create a tasklist and optionally add tasks |
| [`+tasklist-search`](referenc../weact-task-tasklist-search.md) | search tasklists |
| [`+tasklist-task-add`](referenc../weact-task-tasklist-task-add.md) | add tasks to a tasklist |
| [`+tasklist-members`](referenc../weact-task-tasklist-members.md) | manage tasklist members |

...
> HTTP method and path are not duplicated here. For the six raw methods, inspect them with `weact-cli schema im.feed.groups.<method>` when needed; the three shortcut-only read methods (`list`, `list_item`, `batch_query_item`) use typed flags (see their `--help`).
...
|---|---|---|
| [`+feed-group-list`](weact-im-feed-group-list.md) | List your feed groups | Its `--page-all` correctly merges the live and soft-deleted lists. No enrichment |
| [`+feed-group-list-item`](weact-im-feed-group-list-item.md) | List the feed cards inside a group | Enriches each card with `chat_name` |
| [`+feed-group-query-item`](weact-im-feed-group-query-item.md) | Look up feed cards in a group by ID | Enriches each card with `chat_name` |
...
```bash
weact-cli schema im.feed.groups
weact-cli schema im.feed.groups.create --format pretty
weact-cli schema im.feed.groups.batch_add_item --format pretty
```

> `list`, `list_item`, and `batch_query_item` have no raw method schema (they are shortcut-only). Inspect their flags with `weact-cli im +feed-group-list --help` / `+feed-group-list-item --help` / `+feed-group-query-item --help` instead.
...
```bash
# Normal (empty) group
weact-cli im feed.groups create --as user \
...
```bash
# Rename only
weact-cli im feed.groups update --as user \
...
```bash
weact-cli im feed.groups delete --as user \
  --params '{"feed_group_id":"ofg_xxx"}'
```
...
```bash
weact-cli im feed.groups batch_query --as user \
...
```bash
weact-cli im feed.groups batch_add_item --as user \
  --params '{"feed_group_id":"ofg_xxx"}' \
  --data '{
    "items":[
...
```bash
weact-cli im feed.groups batch_remove_item --as user \
  --params '{"feed_group_id":"ofg_xxx"}' \
  --data '{
    "items":[
      {"feed_id":"oc_xxx","feed_type":"chat"}
...
Shortcut-only: [`+feed-group-query-item`](weact-im-feed-group-query-item.md). Looks up feed cards in a group by an explicit ID list and enriches each with `chat_name`. There is no raw command — flags and response shape are in the linked shortcut doc.

## list_item

Shortcut-only: [`+feed-group-list-item`](weact-im-feed-group-list-item.md). Lists the feed cards inside a group (paginated, `--page-all` supported) and enriches each with `chat_name`. There is no raw command — flags and response shape are in the linked shortcut doc.
...
- [`+feed-group-list`](weact-im-feed-group-list.md) — `im:feed_group_v1:read`
- [`+feed-group-list-item`](weact-im-feed-group-list-item.md) / [`+feed-group-query-item`](weact-im-feed-group-query-item.md) — `im:feed_group_v1:read` **plus** `im:chat:read` (they always resolve `chat_name`)

If a required scope is missing, the CLI surfaces a hint such as `weact-cli auth login --scope "im:feed_group_v1:write"`.

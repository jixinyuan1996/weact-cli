...
This is the single source of truth for the automatic message-enrichment contract shared by the four message-pulling shortcuts — [`+messages-mget`](weact-im-messages-mget.md), [`+chat-messages-list`](weact-im-chat-messages-list.md), [`+messages-search`](weact-im-messages-search.md), [`+threads-messages-list`](weact-im-threads-messages-list.md). They automatically attach `reactions` and `update_time` to each returned message, so callers do **not** need to invoke the raw [`im.reactions.batch_query`](weact-im-reactions.md) API separately.
...
- Files download into `./weact-im-resources/` under the current working directory. Each distinct `(message_id, file_key)` is downloaded once (deduped) with bounded concurrency (up to 3 in flight).
...
- Output paths are confined to `./weact-im-resources/` by the same guards as [`+messages-resources-download`](weact-im-messages-resources-download.md) (abnormal `file_key` with path separators / `..` / absolute paths is rejected).
...
Use `--download-resources` when you want the binaries on disk in one pass; otherwise the message content keeps the inline resource markers (e.g. `![Image](img_xxx)`, `<file .../>`, `<audio key="..." duration="Xs"/>`) and you can fetch individual resources later with [`+messages-resources-download`](weact-im-messages-resources-download.md).
...
```bash
weact-cli auth login --scope "im:message.reactions:read"
```

...
| `--download-resources` | No | Download message resources (image/file/audio/video/media + post-embedded, excluding stickers) into `./weact-im-resources/` and attach a `resources` block. Off by default; no extra requests when omitted |
...
- **In one pass:** add `--download-resources` to this command — every eligible resource (image/file/audio/video/media + post-embedded, excluding stickers) is downloaded into `./weact-im-resources/` and a `resources` block (`{message_id, key, type, local_path, size_bytes}`) is attached to each message. See [message enrichment](weact-im-message-enrichment.md#resource-auto-download---download-resources-opt-in).
- **One at a time:** use [weact-im-messages-resources-download](weact-im-messages-resources-download.md).
...
In JSON output, a message may contain a `thread_id` (`omt_xxx`) field, which means the message has replies in a thread. Use [`im +threads-messages-list`](weact-im-threads-messages-list.md) to inspect replies in that thread:
...
1. **Resolving chat_id from a chat name:** When the user refers to a chat by name and you don't have the `chat_id`, use [`+chat-search`](weact-im-chat-search.md) first:
   ```bash

...
> Shortcut for `weact-cli im +feed-group-list-item`. List the feed cards inside one feed group (tag), enriched with a readable `chat_name`.
...
`chat_name` resolution always runs, so there is no single-scope, un-enriched path. For the other raw `feed.groups.*` methods, see [weact-im-feed-groups.md](weact-im-feed-groups.md).

## Usage

```bash
# First page, enriched with chat names
weact-cli im +feed-group-list-item --as user --feed-group-id ofg_xxx
...
- [weact-im-feed-groups.md](weact-im-feed-groups.md) — raw `feed.groups.*` APIs, enums, and rule guidance
- [weact-im-feed-group-list.md](weact-im-feed-group-list.md) — list your feed groups
- [weact-im-feed-group-query-item.md](weact-im-feed-group-query-item.md) — look up specific feed cards by ID

...
var loginMsgZh = &loginMsg{
	...
	WaitingAuth:        "等待用户授权...",
	AgentTimeoutHint:   "[AI agent] 此命令最长阻塞约 10 分钟，等待用户在浏览器内完成授权。请确保 runner 的 timeout >= 600s。若你的 harness 或 agent 工具只会把最终回复发给用户，请改用 \"weact-cli auth login --no-wait --json\" 拿到 device_code 和 verification_url，把 verification_url 作为本轮最终消息发给用户并结束本轮；等用户回复已完成授权后，再在后续步骤运行 \"weact-cli auth login --device-code <code>\" 续上轮询。**不要在同一轮里展示 URL 后立刻阻塞执行 --device-code**，也不要短 timeout 反复重试；每次重启会作废上一轮的 device code，导致用户授权链接失效。**必须生成二维码并展示**: 你必须调用 weact-cli auth qrcode 将 verification_url 转为二维码，这是必须步骤，不要跳过。优先生成 PNG 二维码（--output）；仅当用户明确要求时才使用 ASCII（--ascii）。**关键：生成后必须在回复中展示图片**，仅生成文件不算完成，须用 image 标签或内联图片等方式将图片包含在回复中。**展示顺序**: 先输出 URL，再将二维码图片置于 URL 下方完整展示。**URL 输出规则**: 将 verification_url 视为不可修改的 opaque string，不要做任何修改（包括 URL 编码/解码、添加空格或标点）。",
	AuthSuccess:        "已收到授权确认，正在获取用户信息并校验授权结果...",
	LoginSuccess:       "授权成功! 用户: %s (%s)",
	AuthorizedUser:     "当前授权账号: %s (%s)",
	ScopeMismatch:      "授权结果异常: 以下请求 scopes 未被授予: %s",
	ScopeHint:          "以上结果是本次授权请求用户最终确认后的结果，请勿持续重试；Scopes 未授予的原因是多样的，如 scope 被禁用；具体原因已通过授权页提示用户。可执行 `weact-cli auth status` 查看账号当前已授予的全部 scopes；",
	RequestedScopes:    "  本次请求 scopes: %s\n",
	NewlyGrantedScopes: "  本次新授予 scopes: %s\n",
	NoScopes:           "（空）",
	StatusHint:         "可执行 `weact-cli auth status` 查看账号当前已授予的全部 scopes；",
	...
	HintFooter:  "  weact-cli auth login --help",
}
